// ============================================
// PDF BUILDER — Deutsch für die Pflege · Teil 1 (Körperpflege)
// Claudia Toth · Annettes Deutschkurs · A2.1
// ============================================
(function () {
    if (typeof document === 'undefined') return;
    try { buildPDF(); } catch (e) {
        const r = document.getElementById('pdf-content');
        if (r) r.innerHTML = '<pre style="color:red">ERROR: ' + e.message + '\n' + e.stack + '</pre>';
    }

    function buildPDF() {
        const root = document.getElementById('pdf-content');
        if (!root) return;
        root.innerHTML = buildPflegeWarn() + buildCast() + buildTheory() + buildDialogs() + buildExercises() + buildFlashcards();
    }

    // ============================================
    // BOX ETIC la început
    // ============================================
    function buildPflegeWarn() {
        return `<div class="pflege-warn">
            <h4>⚠️ Notă importantă</h4>
            <p>Această lecție te pregătește <strong>LINGVISTIC</strong> — îți dă propozițiile pe care le auzi și le rostești în fiecare zi cu pacientul. <strong>NU înlocuiește</strong> calificarea profesională (Pflegehelferin / Pflegefachkraft). Pentru îngrijire reală, urmează un curs autorizat de Pflege.</p>
        </div>`;
    }

    // ============================================
    // CAST cu Frau Müller (emoji avatar)
    // ============================================
    function buildCast() {
        return `<div class="cast-banner">
            <h4>👋 Cast „Annettes Deutschkurs" · Andreea devine îngrijitoare</h4>
            <div class="cast-grid">
                <div class="cast-card"><img src="images/andreea.png" alt="Andreea"><div class="name">Andreea 🇷🇴</div><div class="detail">Pflegerin · protagonistă</div></div>
                <div class="cast-card"><div class="frau-mueller-pdf-avatar">👵</div><div class="name">Frau Müller</div><div class="detail">Pacientă · 78 ani</div></div>
                <div class="cast-card"><img src="images/annette.png" alt="Annette"><div class="name">Annette</div><div class="detail">Profesoara · Berlin</div></div>
                <div class="cast-card"><img src="images/mihai.png" alt="Mihai"><div class="name">Mihai</div><div class="detail">Bucătar · Potsdam</div></div>
                <div class="cast-card"><img src="images/florian.png" alt="Florian"><div class="name">Florian</div><div class="detail">Doctor · Berlin</div></div>
                <div class="cast-card"><img src="images/carolina.png" alt="Carolina"><div class="name">Carolina</div><div class="detail">Fotografă · Berlin</div></div>
            </div>
        </div>`;
    }

    // ============================================
    // TEORIA — curățată de elemente interactive
    // ============================================
    function buildTheory() {
        if (typeof theoryHTML !== 'string') return '';
        let t = theoryHTML;
        // Elimină lesson-audio cu anchor sigur
        t = t.replace(/<div class="lesson-audio">[\s\S]*?<\/span>\s*<\/div>/g, '');
        // Elimină butoane onclick rămase
        t = t.replace(/<button[^>]*onclick="[^"]*"[^>]*>[^<]*<\/button>/g, '');
        // Elimină audio tag-uri
        t = t.replace(/<audio[^>]*>[\s\S]*?<\/audio>/g, '');
        // Subsection headers → h2
        t = t.replace(/<div class="subsection-header"[^>]*>\s*<h4>([^<]+)<\/h4>\s*<span class="sub-arrow">[^<]*<\/span>\s*<\/div>/g, '<h2 class="sub-chapter">$1</h2>');
        // Container divs → flatten
        t = t.replace(/<div class="subsection">/g, '<div class="theory-box">');
        t = t.replace(/<div class="sub-section-content"[^>]*>/g, '<div>');
        // Box-uri speciale → box-uri PDF
        t = t.replace(/<div class="capcana-box">/g, '<div class="theory-box warn-box">');
        t = t.replace(/<div class="grammar-tip">/g, '<div class="theory-box info-box">');
        t = t.replace(/<div class="trennbare-recap-box">/g, '<div class="theory-box">');
        t = t.replace(/<div class="vocab-bonus">/g, '<div class="theory-box">');
        t = t.replace(/<div class="final-note-box">/g, '<div class="theory-box warn-box">');
        return `<h1 class="chapter new-section">📘 1. Teorie — Körperpflege: propozițiile esențiale</h1>` + t;
    }

    // ============================================
    // DIALOGURI — transcript bilingv pentru ambele
    // ============================================
    function buildDialogs() {
        let html = `<h1 class="chapter new-section">🎬 2. Dialoguri — Andreea ↔ Frau Müller</h1>`;
        [dialog1Data, dialog2Data].forEach((d, idx) => {
            if (!d) return;
            html += `<div class="ex-block">
                <h3>${idx === 0 ? 'Dialog 1' : 'Dialog 2'}: „${d.title}"</h3>
                <div class="instruction">${d.context}</div>
                <div class="dialog-pdf-card">`;
            d.replici.forEach((r, ri) => {
                const spkrName = r.speaker === 'andreea' ? '🧑‍⚕️ Andreea' : '👵 Frau Müller';
                html += `<div class="reply"><span class="spkr">${ri + 1}. ${spkrName}:</span> <span class="de"> ${r.de}</span><br><span class="ro">🇷🇴 ${r.ro}</span></div>`;
            });
            html += `</div></div>`;
        });
        return html;
    }

    // ============================================
    // EXERCIȚII — toate 6 cu rezolvări (Ex 7 e Schreiben, fără răspuns fix)
    // ============================================
    function buildExercises() {
        let html = `<h1 class="chapter new-section">📝 3. Exerciții — cu rezolvări complete</h1>`;

        // EX 1: Match DE → RO
        if (typeof ex1Data !== 'undefined') {
            html += `<div class="ex-block"><h3>Übung 1: Match — Propoziții germane → Traducere română</h3>
                <div class="instruction">Pentru fiecare propoziție DE, scrii traducerea în RO.</div>
                <div class="rezolvare-banner">✓ Rezolvare</div>
                <table><thead><tr><th style="width:50%">🇩🇪 Germană</th><th>🇷🇴 Română</th></tr></thead><tbody>`;
            ex1Data.forEach((it, i) => {
                html += `<tr><td class="verb">${i + 1}. ${it.de}</td><td class="ro-text">${it.correct}</td></tr>`;
            });
            html += `</tbody></table></div>`;
        }

        // EX 2 + 4 + 5: Fill-in (sentence cu blank → correct)
        if (typeof ex2Data !== 'undefined') html += fillInBlock('Übung 2: Verbe trennbare în context Pflege', 'Completează cu forma corectă. Prefixul „zboară" la final!', ex2Data);
        if (typeof ex4Data !== 'undefined') html += fillInBlock('Übung 4: Du-Form → Sie-Form (Imperativ politicos)', 'Transformă forma cu „du" (informal) în forma politicoasă cu „Sie".', ex4Data);
        if (typeof ex5Data !== 'undefined') html += fillInBlock('Übung 5: Dialog Gap-Fill — Completează replicile lipsă', 'Replicile lipsă din dialogul Andreea ↔ Frau Müller.', ex5Data);

        // EX 3: Dictat — afișează doar răspunsurile (audio nu se redă în PDF)
        if (typeof ex3Data !== 'undefined') {
            html += `<div class="ex-block"><h3>Übung 3: Audio Dictat — Hedda spune, tu scrii</h3>
                <div class="instruction">Ascultă fraza Heddei și scrie-o exact. (Audio disponibil online — PDF afișează doar răspunsurile pentru recap.)</div>
                <div class="rezolvare-banner">✓ Rezolvare</div>`;
            ex3Data.forEach((it, i) => {
                html += `<div class="ex-item"><span class="ex-num">${i + 1}</span><div class="ex-body"><div class="ex-a">${it.correct}</div></div></div>`;
            });
            html += `</div>`;
        }

        // EX 6: Traducere RO → DE
        if (typeof ex6Data !== 'undefined') {
            html += `<div class="ex-block"><h3>Übung 6: Traducere RO → DE (propoziții Pflege)</h3>
                <div class="instruction">Scrie propoziția în germană.</div>
                <div class="rezolvare-banner">✓ Rezolvare</div>
                <table><thead><tr><th style="width:50%">🇷🇴 Română</th><th>🇩🇪 Germană</th></tr></thead><tbody>`;
            ex6Data.forEach((it, i) => {
                html += `<tr><td class="ro-text">${i + 1}. ${it.ro}</td><td class="verb">${it.correct}</td></tr>`;
            });
            html += `</tbody></table></div>`;
        }

        // EX 7: Schreiben (doar prompt + model)
        html += `<div class="ex-block"><h3>✍️ Übung 7: Schreiben — Cum o ajuți pe Frau Müller dimineața</h3>
            <div class="instruction">Imaginează-ți că ești îngrijitoare la Frau Müller. Descrie în 5-7 propoziții cum decurge o rutină de dimineață cu ea.</div>
            <p><strong>Verbe utile:</strong> helfen · waschen · trocknen · ausspülen · eincremen · auftragen · kämmen · anziehen · anfangen</p>
            <p><strong>Structuri utile:</strong> Ich helfe Ihnen bei... · Ich wasche Ihnen den/die/das... · Soll ich...? · Brauchen Sie...?</p>
            <div class="rezolvare-banner">📝 Model de răspuns</div>
            <p><em>Ich gehe um 7 Uhr ins Zimmer von Frau Müller. Ich sage: „Guten Morgen, Frau Müller! Wie haben Sie geschlafen?" Dann frage ich: „Brauchen Sie meine Hilfe bei der Körperpflege?" Wenn sie ja sagt, bringe ich die Waschschüssel ins Zimmer. Ich wasche ihr den Rücken und die Füße. Danach trockne ich sie ab und trage Hautschutzcreme auf. Zum Schluss kämme ich ihr die Haare.</em></p>
            <p class="ro-translation">🇷🇴 Merg la ora 7 în camera Doamnei Müller. Spun: „Bună dimineața, Doamnă Müller! Cum ați dormit?" Apoi întreb: „Aveți nevoie de ajutorul meu la igiena corporală?" Dacă spune da, aduc ligheanul în cameră. Îi spăl spatele și picioarele. După aceea o șterg și aplic cremă de protecție. La sfârșit îi pieptăn părul.</p>
            </div>`;

        return html;
    }

    function fillInBlock(title, instruction, data) {
        let h = `<div class="ex-block"><h3>${title}</h3><div class="instruction">${instruction}</div><div class="rezolvare-banner">✓ Rezolvare</div>`;
        data.forEach((it, i) => {
            const filled = it.sentence.replace(/_{2,}/g, `<strong style="color:#047857">${it.correct}</strong>`);
            h += `<div class="ex-item"><span class="ex-num">${i + 1}</span><div class="ex-body"><div class="ex-q">${filled}</div>${it.translation ? `<div class="ex-explanation">🇷🇴 ${it.translation}</div>` : ''}</div></div>`;
        });
        return h + `</div>`;
    }

    // ============================================
    // FLASHCARDS — 48 cards în 2 coloane
    // ============================================
    function buildFlashcards() {
        const count = (typeof flashcardsData !== 'undefined') ? flashcardsData.length : 0;
        let html = `<h1 class="chapter new-section">📇 4. Vocabular complet (Flashcards)</h1>
            <p style="margin-bottom:10px">Cele <strong>${count} carduri</strong> ale lecției grupate pe categorii: Verbe · Părți corp · Obiecte · Imperativ · Întrebări · Modele.</p>
            <div class="flashcards-grid">`;
        if (typeof flashcardsData !== 'undefined') {
            flashcardsData.forEach(card => {
                html += `<div class="fc-row"><span class="de">${card.de}</span><span class="ro">— ${card.ro}</span></div>`;
            });
        }
        html += `</div>`;
        return html;
    }
})();
